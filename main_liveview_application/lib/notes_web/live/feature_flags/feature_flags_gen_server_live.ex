defmodule NotesWeb.FeatureFlags.FeatureFlagsGenServerLive do
  use NotesWeb, :live_view
  use Notes.PubSub
  alias Notes.Repository.Protocol.FeatureFlagRepo
  alias Notes.Repository.FeatureFlagGenServerRepo
  ...
  def mount(_, _, socket) do
    if connected?(socket) do
      ...
      repo = FeatureFlagGenServerRepo
      FeatureFlagRepo.seed(repo)

      {:ok, socket |> assign(feature_flags: [], repo: repo)}
    else
  ...
  def handle_info(@feature_flag_changed_topic, socket) do
    {:noreply, socket |> assign(feature_flags: FeatureFlagRepo.get_all(socket.assigns.repo))}
  end

  def handle_event("open_features_tab", _, socket) do
    {:noreply,
     push_event(socket, "open_tab", %{
       url: "http://localhost:4000/feature_flags/features?use_gen_server=true"
     })}
  end

  def handle_event("toggle_flag", %{"name" => name}, %{assigns: %{repo: repo}} = socket) do
    FeatureFlagRepo.toggle(repo, name)
